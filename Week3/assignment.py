...
def processCrud(numberCPU, image_files, green_files, process_files):
  tasks = [*zip(image_files,green_files,process_files)]
  start_time = timeit.default_timer()
  with mp.Pool(numberCPU) as p:
    p.starmap(create_new_frame, tasks)
...
if __name__ == '__main__':
    ...
    for x in range(1,301):
      image_files.append(rf'elephant/image{x:03d}.png')
    green_files = []
    for x in range(1,301):
      green_files.append(rf'green/image{x:03d}.png')
    process_files = []
    for x in range(1,301):
      process_files.append(rf'processed/image{x:03d}.png')

    for x in range(1,CPU_COUNT+1):
      timee = processCrud(x,image_files,green_files, process_files)
      xaxis_cpus.append(x)
      yaxis_times.append(timee)
