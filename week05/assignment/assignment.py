...
from cse251 import *
from matplotlib.pyplot import bar, semilogx
...
class Factory(threading.Thread):
    ...
    def __init__(self,q, bar,semma,ID, stats):
    ...
    def run(self):
        ...
        for i in range(self.cars_to_produce):
            with self.s:
                c = Car()
                if self.qu.get_max_size() < MAX_QUEUE_SIZE:
                    self.qu.put(c)
                    self.st[self.i] = self.st[self.i]+1
...
class Dealer(threading.Thread):
    ...
    def __init__(self,q, loc, sem, dealID, status):
        threading.Thread.__init__(self)
        self.qu = q
        self.l = loc
        self.s = sem
    ...
    def run(self):
        while True:
            ...
            with self.s:
                with self.l:
                    if self.qu.get_max_size() > 0:
                        carrr = self.qu.get()
                        if carrr == 'DONE':
                            self.qu.put('DONE')
                            break
                        self.stat[self.dealID] = self.stat[self.dealID]+1
...
def run_production(factory_count, dealer_count):
    ...
    semFac = threading.Semaphore(factory_count)
    semDeal = threading.Semaphore(dealer_count)
    # TODO Create queue
    que = Queue251()
    # TODO Create lock(s)
    lock = threading.Lock()
    # TODO Create barrier(s)
    facBarrier = multiprocessing.Barrier(factory_count)
    ...
    for x in range(factory_count):
        factories.append(Factory(que,facBarrier,semFac,x,factory_stats))
    # TODO create your dealerships
    dealers = []
    for x in range(dealer_count):
        dealers.append(Dealer(que,lock,semDeal,x,dealer_stats))
    log.start_timer()
    ...
    return (run_time, que.get_max_size(), dealer_stats, factory_stats)
