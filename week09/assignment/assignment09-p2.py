...
def solve_find_end(maze, startx, starty):
    """ finds the end position using threads.  Nothing is returned """
    # When one of the threads finds the end position, stop all of them
    color = get_color()
    with cond:
        while True:
            moves = maze.get_possible_moves(startx,starty)
            if len(moves) > 1:
                threads = []
                for x in range(len(moves) -1):
                    threads.append(threading.Thread(target=solve_find_end, args= (maze, moves[x][0], moves[x][1])))
                for x in threads:
                    x.start()
                pass
            elif len(moves) == 1:
